...
% Manuscript SHA256: 7c26890d3cefad836c38903ef0fcab25c12b03acbaf6941f24f64d5e4282a6bb
...
\urlstyle{same}
\hypersetup{
  pdftitle={ArdEVO: Orchestrated Neuroevolution with a Persistent, Compounding Solution Library},
  hidelinks,
...
\section{3 The Icarus Ladder}\label{3-the-icarus-ladder}

Icarus is the data contract the whole system is built against: a single typed interface under which every task, from XOR to ARC, is a differentiable supervised problem. The dataset is generated by a separate open repository and published on the Hugging Face Hub (\path{Ardea/Icarus-dataset}); ArdEVO vendors only the loader/encoder runtime.

\subsection{3.1 The task contract}\label{31-the-task-contract}

A \textbf{Field} is a tensor plus the descriptor an encoder needs: \texttt{axes} (one semantic axis per dimension, from \{EXAMPLE, CHANNEL, TIME, HEIGHT, WIDTH, DEPTH, EXTRA\}), a \texttt{value\_type} in \{BINARY, CATEGORICAL, MULTILABEL, CONTINUOUS, ORDINAL\}, \texttt{n\_classes} where class-bearing, an optional \texttt{value\_range} for continuous normalization, and an optional boolean mask marking padding. A \textbf{Task} is a non-empty \texttt{support} list and a \texttt{query} list of \texttt{(input\_Field,\ output\_Field)} pairs: the inner loop fits on support and is scored on the held-out query. Tasks with authoritative native splits (XOR, ARC) are marked \texttt{fixed\_split} and are never re-split; bucketed rungs re-split reproducibly under a crc32-stable seed so the same task yields the same split across runs and processes.
...
\tightlist
\item
  \textbf{Loss dispatch never sees the task\textquotesingle s identity.} Cross-entropy for CATEGORICAL/ORDINAL, BCE-with-logits for BINARY/MULTILABEL, MSE for CONTINUOUS, uniformly mask-weighted. One gradient inner loop therefore spans all 18 rungs.
\item
  \textbf{Structural signatures only.} A task\textquotesingle s library identity is \texttt{value\_type\textbar{}axes} plus flattened widths, derived purely from field descriptors, deliberately ignoring rung, name, and kind. Any future task source with the same structural shape (the ARC-AGI-3 adapter case) hits the same library entries. This is the portability constraint that shapes the entire memory design.
\item
  \textbf{Interactive provenance is not a regime.} Rungs 4 and 5 derive from policy rollouts but are stored and scored as ordinary differentiable pairs; no environment simulator exists anywhere in the consumer.
\end{itemize}

The reference \textbf{Level0Encoder} flattens inputs to \texttt{{[}batch,\ width{]}}, min-max normalizes CONTINUOUS fields into {[}0, 1{]}, zeroes padded positions, and fits to a target width; a \textbf{TemporalEncoder} rebuilds the TIME axis that Level0 flattens (inputs become \texttt{{[}batch,\ time,\ features\_per\_step{]}}) and is pinned bitwise-equivalent to Level0 at T=1 so temporal and flat scores are directly comparable.

\subsection{3.2 The rungs}\label{32-the-rungs}
...
Tasks
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Contract (input -\textgreater{} output, flattened widths)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Note
\end{minipage} \\
\midrule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
1 & \texttt{xor} & 1 & BINARY 2 -\textgreater{} BINARY 1 & fixed split 4/4; minimal 2-\textgreater1 topology caps at 75\%, so a solve must grow structure \\
2 & \texttt{parity.n4..n8} & 9 & BINARY \textless=8 -\textgreater{} BINARY 1 & n4 has 16 examples total; query is anti-generalization at n4 \\
3 & \texttt{two\_spirals} & 1 & CONTINUOUS 2 -\textgreater{} CATEGORICAL 2 & fixed split 194/192; the generalization case study of Section 7 \\
4 & \texttt{pole.*} & 39 & CONTINUOUS 4 per step -\textgreater{} 1 (temporal) & policy-rollout provenance, differentiable regression \\
5 & \texttt{double\_pole.*} & 39 & CONTINUOUS 6 per step -\textgreater{} 2 (temporal) & as rung 4 \\
6 & MNIST / Fashion-MNIST & 1,200 & CONTINUOUS 784 -\textgreater{} CATEGORICAL 10 & first real image statistics \\
7 & CIFAR & 1,000 & CONTINUOUS 3,072 -\textgreater{} CATEGORICAL 10 & the wide-image stress point \\
8 & \texttt{ecg} & 1,287 & CONTINUOUS 1,000 -\textgreater{} CATEGORICAL 4 (temporal) & NAS-Bench-360 \\
9 & \texttt{satellite} & 15,625 & CONTINUOUS 46 -\textgreater{} CATEGORICAL 24 (temporal) & NAS-Bench-360 \\
10 & \texttt{ninapro} & 62 & CONTINUOUS 832 -\textgreater{} CATEGORICAL 18 (temporal) & NAS-Bench-360 \\
11 & \texttt{spherical} & 938 & CONTINUOUS 10,800 -\textgreater{} CATEGORICAL 100 & NAS-Bench-360 \\
12 & \texttt{cosmic} & 512 & CONTINUOUS 65,536 -\textgreater{} BINARY 65,536 & NAS-Bench-360; dense output \\
13 & \texttt{darcy\_flow} & 32 & CONTINUOUS 177,241 -\textgreater{} CONTINUOUS 177,241 & NAS-Bench-360; dense output \\
14 & \texttt{psicov} & 58 & CONTINUOUS 271k to \textgreater=13.97M -\textgreater{} CONTINUOUS 4.8k to \textgreater=245k & NAS-Bench-360; widths vary per task; upper entries are observed, not maxima \\
15 & \texttt{fsd50k} & 12,529 & CONTINUOUS 5,632 -\textgreater{} MULTILABEL 200 & NAS-Bench-360 \\
16 & \texttt{deepsea} & 3,489 & CONTINUOUS 4,000 -\textgreater{} MULTILABEL 36 (temporal) & NAS-Bench-360 \\
17 & RAVEN / PGM & 23,294 & CONTINUOUS 409,600 -\textgreater{} CATEGORICAL 8 & abstract relational reasoning \\
18 & ARC-AGI & 800 & CATEGORICAL 900 -\textgreater{} CATEGORICAL 9,000 & native demo/test split; the target regime \\
\end{longtable}

Task counts and I/O widths were enumerated live from the published dataset on 2026-07-06 (the probe transcript is archived with the frozen artifacts). Widths are the flattened Level0 view that library signatures key on; temporal rungs additionally unroll the TIME axis under the stepped substrate. Rung difficulty is intrinsic to the data, not to any code path: higher rungs mostly mean wider flattened I/O and harder generalization, not new mechanisms. Those archived probes measure rung 11 (\texttt{spherical}) at 10,800 inputs by 100 outputs, rung 12 (\texttt{cosmic}) at 65,536 by 65,536, rung 13 (\texttt{darcy\_flow}) at 177,241 by 177,241, and rung 14 (\texttt{psicov}) from 271,377 to 5,095,857 inputs and 4,761 to 89,401 outputs. The 2026-07-12 preflight subsequently observed a larger Psicov task at 13,966,425 by 245,025, so the table reports observed lower bounds rather than a closed maximum. These rungs exceed the minimal-initialization regime (Section 10).

\subsection{3.3 Pool construction and honesty tooling}\label{33-pool-construction-and-honesty-tooling}

A run\textquotesingle s task pool loads rung by rung, each in its own dataset handle, so one unloadable rung cannot kill or silently narrow a run: every failure or empty rung is recorded as a first-class \texttt{SkippedRung} row in the run summary, and the shipping configuration sets \texttt{require\_all\_rungs\ =\ true}, which aborts loudly instead. A standalone probe (\texttt{uv\ run\ rung\_doctor}) reports per-rung loadability, task counts, I/O signatures, temporality, and how many library entries match each rung\textquotesingle s shape exactly or within tolerance. This tooling exists because an early "full ladder" run silently never scheduled rung 5, and nothing said so; we treat schedule coverage as a reportable result, not an assumption.
...
\subsection{5.1 Genome and substrate}\label{51-genome-and-substrate}
...
\tightlist
\item
  \textbf{Product aggregation.} A node aggregates its inputs by sum or product; product nodes make gating and second-order interactions evolvable as single units rather than as trained approximations.
\item
  \textbf{Recurrent edges.} A connection may be time-delayed (reading the previous step), exempt from acyclicity, inert under the plain feedforward decode, and active under the stepped substrate used for TIME-axis tasks (trained by ordinary backpropagation through time).
\item
  \textbf{Refinement depth.} Each genome carries \texttt{refine\_steps}: how many times the decoded network re-applies itself to a static input, threading latent state through hidden-to-hidden recurrent edges and feeding the current answer back through output-to-hidden edges. This is the evolutionary port of tiny-network recursive reasoning (Jolicoeur-Martineau, 2025): depth of computation becomes an evolvable, per-genome quantity severed from parameter count. \texttt{refine\_steps\ =\ 1} is byte-identical to feedforward, so the lever is free until selected for.
\item
  \textbf{Macro genes.} A macro embeds an entire library entry as one frozen unit with one innovation marker: an atomic crossover and speciation unit, resolved at decode time against the live library, with hidden identity stubs for its outputs that every mutation operator is forbidden to retarget. Macros are how the search reuses a discovered network wholesale, the analogue of dropping in a hand-designed cell, except nothing about the cell was hand-designed. All module and composition expansion obeys one configurable reference-depth invariant: the selected root payload is depth 0 and free, while each \texttt{library:\textless{}key\textgreater{}} dereference across either kind of boundary consumes exactly one level. \texttt{{[}evolution.composition{]}\ max\_inline\_depth} defaults to 4 in code and in the smoke overlay; the canary methodology and its preflight, brute, lattice, and full-cluster derivatives use 8. Cycles are rejected independently of this cap.
\end{itemize}

Genomes decode through one router: \texttt{refine\_steps\ \textgreater{}\ 1} yields the refinement substrate, TIME-axis adapters yield the stepped recurrent substrate, and everything else yields the plain feedforward substrate. All three compile the genome into level-ordered dense matrix multiplies with a \textbf{compact-column layout}: weight and mask matrices allocate columns only for computed nodes, so a 784-input MNIST genome with a dozen hidden nodes pays for roughly twenty columns (its hidden and output nodes), not 800. The layout is pinned bitwise-identical to the dense reference implementation and is the difference between 400 ms and 30 ms per candidate training call at width 784 (Section 8). Complexity is structural: enabled edges plus hidden nodes plus macro placements.

\subsection{5.2 The mutation menu}\label{52-the-mutation-menu}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{add\_rich\_node} & new hidden node wired from up to 4 random sources and to every output; multi-input, so gradient training can make it useful immediately (single-edge splits add no capacity on parity-class tasks) \\
\texttt{add\_deep\_node} & as \texttt{add\_rich\_node}, plus edges into existing hidden nodes: the depth operator that two-spirals-class tasks require \\
\texttt{mutate\_activation} & reassign one hidden node\textquotesingle s activation from the palette \{tanh, relu, sigmoid, identity, sin, gaussian\}; the only entry route for periodic and radial primitives, which are never seeded \\
\path{mutate_aggregation} & flip a node between sum and product (bounded fan-in for products) \\
\path{add_recurrent_connection} & add a time-delayed edge, biased toward self-loops (the accumulator primitive) \\
\path{tweak_refine_steps} & nudge the genome\textquotesingle s refinement depth by one within {[}1, 6{]} \\
\path{add_library_module} & inline a library entry as unfrozen, evolvable structure with fresh identities \\
\texttt{add\_macro\_node} & place a library entry as one frozen macro unit \\
\texttt{remove\_connection}, \path{remove_hidden_node} & true gene deletion, so lineages can climb back down in size (selected in the probe and all-features variants) \\
\texttt{add\_local\_node}, \path{add_local_connection}, \texttt{add\_shared\_motif} & the geometry trio (below) \\
\end{longtable}
...
\textbf{Geometry and weight-sharing operators.} Input nodes on grid-shaped tasks are stamped with raw axis-index coordinates. \path{add_local_connection} weights candidate edges by coordinate distance, \texttt{add\_local\_node} grows a neighborhood detector, and \texttt{add\_shared\_motif} replicates an evolved detector at other neighborhoods. The current configuration ties corresponding replicated edges into one trainable weight group, a generic translation-sharing mechanism whose template is discovered rather than supplied; \path{untie_motif_weights} dissolves one group without changing the function at birth, allowing specialization to evolve. Incomparable banks are never treated as neighbors, and all geometry operators are no-ops off-grid.
...
\subsection{5.5 Evaluation and the robustness currency}\label{55-evaluation-and-the-robustness-currency}
...
\subsection{6.1 Entries, keys, and two notions of identity}\label{61-entries-keys-and-two-notions-of-identity}
...
\subsection{6.4 The motif census}\label{64-the-motif-census}

The structural fingerprint keeps node identifiers verbatim, so it cannot recognize that two entries grew the same four-node gadget under different numbering. The \textbf{motif census} fills that gap: each payload becomes a labeled dataflow graph, connected k-subgraphs up to k = 5 are enumerated exactly by ESU with deterministic truncation, and each subgraph reduces to an exact permutation-canonical form. This raw frequency census is useful for grammar induction and descriptive inventory; its atlas is \textbf{not evidence of architectural discovery}. The post-canary discovery layer excludes pure input/output plumbing, locks candidates from support/library evidence before reading query outcomes, and ranks them against 64 deterministic label- and degree-preserving null graphs using independent lineage support, within-signature performance percentile, robustness percentile, and reuse percentile. \texttt{Observed} requires surprise z \textgreater= 2 and positive performance association; \texttt{functional} additionally requires a frozen-edge knockout loss of at least 0.02 that is worse than 95\% of 16 matched controls after at most 25 recovery steps; \texttt{replicated} requires functional evidence in at least two lineage roots. No candidate is called a new cell, convolution, or architectural invention without replicated evidence. Motif counting follows Milo et al. (2002); the enumerator is Wernicke\textquotesingle s ESU (Wernicke, 2006).
...
\section{7 Experiments}\label{7-experiments}
...
\subsection{7.1 Setup}\label{71-setup}

The results below use the immutable archived configurations listed in Appendix C, not the repository\textquotesingle s later shipping defaults. The headline arm uses interleaved rungs, up to 400 examples per task (support fraction 0.8), query-accuracy acceptance at 0.95, the historical \texttt{routed,\ direct,\ composition} ladder with shares 0.2/0.5/0.3 over 240 generations at depth 0, refinement budget 24, wall ledger floor 0.45, archive admission, minimal initialization, tournament selection, hybrid evaluation, and a 64-member direct population. The gate experiments use the named single-flip variants in Appendix B. This distinction prevents a method improvement made after an experiment from silently rewriting that experiment\textquotesingle s protocol.

\subsection{7.2 Ladder compounding}\label{72-ladder-compounding}
...
3 \texttt{two\_spirals} & 67 & 0 & 67 failed & 0.901 \\
4 \texttt{pole} & 67 & 66 & 3 evolved, 59 hits, 4 refined, 1 failed & 1.0 \\
5 \texttt{double\_pole} & 66 & 66 & 7 evolved, 56 hits, 3 refined & 1.0 \\
6 \texttt{mnist} / \texttt{fashion\_mnist} & 66 & 0 & 66 failed & 0.900 \\
\end{longtable}
...
\textbf{The diagnostic.} For every champion, the hybrid evaluator reports \path{max_sample_accuracy}: the best query accuracy the topology achieves when all trainable weights are set to a single shared value, over six values in {[}-2, 2{]} (the WANN measurement; Gaier and Ha, 2019). A topology that scores well under shared weights carries the solution in its structure; one at chance carries it entirely in trained weights. The pre-registered rule: median below 0.70 and maximum below 0.80 across champions reads as a \textbf{representation wall} (the encoding cannot cheaply express the needed function), as opposed to a search or training failure.
...
\textbf{G1, divergent selection (flip: NSGA-II over {[}support accuracy, novelty, connection cost{]}).} Same seed, same budgets, verified single-flip diff. Trained max rose 0.656 to \textbf{0.792}, last-5-attempt mean 0.571 to 0.699, G1 beat G0 at 14 of 20 attempt indices, with fewer generations (614 versus 730), and produced the first attempts in the project\textquotesingle s records above the prior 0.672 orchestrated reference. The stone lineage went hierarchical where G0\textquotesingle s stayed flat: a level-1 sin-plus-product module (0.677) was extended to a level-2 module (0.792) that was absorbed into the module pool mid-run, and the lineage was capped by a level-3 composition (0.792). And yet: \path{max_sample_accuracy} stayed at chance in both arms. Divergent selection improves what training finds, not what structure encodes.
...
Zero solves at 0.95 anywhere in that table (Figure 3 plots the per-attempt trajectories; Figure 4 places every measured champion against the wall thresholds). The wall has moved twice (from representation toward search, then from search toward per-candidate training economics), and the trained plateau has climbed 0.26. The structure-only statistic has barely moved: at chance in every measured champion but one, and that one (0.688, the stone-seeded snapshot\textquotesingle s best champion, with weight robustness 0.673) still sits below the pre-registered 0.70 wall threshold. We consider that near-immobility the study\textquotesingle s most important negative result and the clearest open problem for encoding research on this system, with the single above-chance reading, produced by the deepest stone-compounded lineage, as the one hint that compounding may eventually push signal into structure.

\pandocbounded{\includegraphics[keepaspectratio]{../../figures/fig3_spirals.png}}

\emph{Figure 3: Two-spirals trained query accuracy per attempt for the gate arms of Section 7.3: G0 (scalar baseline), G1 (NSGA-II over accuracy, novelty, and wiring cost), the warm G1 continuation on the surviving library, and the interrupted probe, with the 0.95 accept bar marked. Stepping stones lift the warm arm to a 0.80 opening on its first attempt; no arm crosses the bar.}

\pandocbounded{\includegraphics[keepaspectratio]{../../figures/fig4_wall.png}}

\emph{Figure 4: Structure versus weights for every champion that ran the hybrid evaluator (222 rows across the completed run, the G0/G1/warm/probe arms, and the stone-seeded snapshot). The x axis is the shared-weight diagnostic (max\_sample\_accuracy, the WANN measurement); the y axis is the trained metric; dashed lines mark the pre-registered 0.70 and 0.80 wall thresholds. Solved xor and parity champions sit at or beyond the wall lines, so their structure carries the function; the two-spirals column is pinned at chance with the single 0.688 exception; continuous-output and ten-class families score near zero under shared weights, where the tolerance-banded and many-class metrics are naturally punishing.}

\subsection{7.4 What the library contains}\label{74-what-the-library-contains}

The snapshot run\textquotesingle s 14-entry library (archived under \path{ai/archive/20260705_g2/}): 13 modules and 1 composition; nine level-1, four level-2, one level-3 (five entries at level 2 or above); four tombstoned by refinement dominance; one dependency entry (the two-spirals stone). Usage statistics separate workhorses from shelf-warmers: the stone accumulated 742 attributed uses without ever being a false hit, and a parity module is referenced by two level-2 macro-bearing modules, one of which the level-3 module references in turn. The motif census over that library\textquotesingle s nine live modules finds 138 distinct size-3/4 motifs, the most supported appearing in nine of nine scanned modules; by diversity class, 111 are mixed, 24 macro-bearing, and 3 uniform-tanh, with no recurrent or gated motifs surviving on this parity-and-pole-dominated shelf (an earlier 37-entry library, grown when two-spirals stones were richer, carried 93 recurrent and 211 gated motifs among 1,496). The census over the completed run\textquotesingle s frozen library (24 live mineable entries, 22 modules and 2 compositions, of the 35 the index holds after garbage collection) is richer (Figure 6): 358 distinct size-3/4 module motifs, the most supported appearing in 19 of the 22 scanned modules, distributed as 135 mixed, 121 macro-bearing, 61 gated, 32 gated-plus-macro, and 9 uniform-tanh, plus one composition-level motif. Two readings stand out. Gated structure survives at this scale (93 of the 358 recurring motifs involve product aggregation) where the smaller snapshot shelf carried none, and macro-bearing motifs dominate exactly as five levels of composition depth would predict. At these scales the census still reads mostly as plumbing statistics; its intended payoff, spotting a repeated evolved gadget across independent solutions, is exactly what the warm-G1 seven-macro artifact previews, and evaluating it on a hundred-entry library is part of the forward program.

\includegraphics[width=0.47\linewidth,height=0.35\textheight]{../../figures/fig5a_overmind.png} \includegraphics[width=0.47\linewidth,height=0.35\textheight]{../../figures/fig5b_seven_macro_artifact.png}\\
\includegraphics[width=0.47\linewidth,height=0.35\textheight]{../../figures/fig5c_level3_chain.png} \includegraphics[width=0.47\linewidth,height=0.35\textheight]{../../figures/fig5d_probe_champion.png}

\emph{Figure 5: Library artifacts, rendered from the archived library states. (a) The routed-substrate portrait at the end of the completed run: a single synced expert vertex with zero lifetime gate traffic, the honest picture behind \texttt{routed\_solved\ =\ 0}. (b) The warm-G1 seven-macro artifact (trained metric 0.807, structural complexity 903): six frozen copies of the same level-1 sin-plus-product gadget plus a level-2 module, repetition assembled one macro at a time at linear structural cost. (c) The snapshot run\textquotesingle s three-deep macro reuse chain: a level-3 parity module embedding a level-2 module embedding a level-1 module. (d) The probe\textquotesingle s best two-spirals champion (query accuracy 0.917), admitted below the bar as a wall-ledger stepping stone.}

\pandocbounded{\includegraphics[keepaspectratio]{../../figures/fig6_motifs.png}}

\emph{Figure 6: Descriptive motif-frequency atlas over the completed lower-ladder run\textquotesingle s frozen library. It is an inventory and grammar-induction input, not evidence that the system discovered a functional architectural primitive.}

\subsection{7.5 Full-ladder canary}\label{75-full-ladder-canary}

The frozen run \path{20260712_145535_orchestrated} completed all 180 scheduled tasks (ten per rung) in 49,311.5 recorded task-seconds and 3,987 generations. Outcomes were 74 evolved, 64 failed, 36 library hits, 4 refinements, and 2 decompositions. Held-out values were recorded for 137 tasks. The library admitted 84 entries during the run and retained 69 after garbage collection. Appendix E gives the complete rung table; all aggregates use held-out query accuracy as primary and show support accuracy separately.

Generalization was strong on rungs 1, 2, 4, 12, and 15: mean held-out accuracies were 1.0000, 0.9846, 0.9525, 0.9840, and 0.9846, respectively. This did not extend uniformly upward. Fashion-MNIST/MNIST support averaged 0.9838 while query averaged 0.5100; CIFAR support averaged 0.7350 while query averaged 0.1531; PGM support was 1.0000 while query averaged 0.1077. ARC support averaged 0.9994, but held-out \textbf{cell-level} accuracy averaged 0.5128 and peaked at 0.84375. The run did not contemporaneously record exact-grid ARC scores, so these rows cannot support an ARC task solve; high background-dominated cell accuracy would not imply one in any case.
...
All remedies motivated by the canary are \textbf{post-canary method changes}, not measured outcomes: compact fixed port maps replace dense decomposition glue; executable below-bar routed distillations become deduplicated composition seeds with strategy metrics; router format v2 keeps the complete gate embedding table resident but decodes selected experts and loads wide adapter/head shards on demand, adds activated parameters to the optimizer, and evicts only after a durable between-task checkpoint; archive format v2 deduplicates content by hash; and motif discovery now requires null and counterfactual evidence. A matched rerun is required to measure any effect.

\subsection{7.6 Pending full-cluster and ablation protocol}\label{76-pending-full-cluster-and-ablation-protocol}

The repository now contains a code-verified scaling protocol whose outcomes are \textbf{pending and are not results in this paper}. \path{configs/canary.toml} is the single full-method definition; \path{configs/full_cluster.toml} extends it with 20 tasks on each of 18 rungs (360 per run), selects \texttt{ClusterCUDA}, and explicitly selects population CUDA training (minimum 768 nodes, maximum padded size 8,192, adaptive microbatch splitting). Its depth-0-through-8 recursion budgets are 2000/1000/500/250/125/62/31/15/7 generations; each attempt has a 7,200-second cap and the entire recursive solve has a 21,600-second cumulative cap. Routed distillation permits 16 nodes; library-archive caps rise to 4 entries per niche and 48 per signature; adaptive direct/allocation guards use 55\% of available host memory less 8 GiB and 70\% of free CUDA memory less 2 GiB, further bounded by scheduler cgroups; composition glue is compact float32 and permits eight library-reference boundaries. Content-addressed snapshots are scheduled after every task with two retained locally. Run/library locks exclude live manual snapshots, final completion requires a durable \texttt{done} receipt, and restore verifies all hashes before atomically installing the recovered tree. The config\textquotesingle s campaign table is metadata; \path{configs/campaigns/full_cluster.toml} materializes isolated cold-library runs for seeds 0, 1, and 2 with one GPU, 16 CPUs, 128 GiB, and a 48-hour job cap each, with a Slurm-array concurrency default of three. A 48-hour allocation may not finish a run; resubmitting the same suite resumes the hash-matched checkpoint and cold-library lineage.

The paper-core ablation manifest contains 45 planned runs over rungs 1 through 5: six P0 arms at five seeds each (full, no routing, no decomposition, no hierarchy, no refinement, and no library reuse) plus five P1 arms at three seeds each (no curriculum, no self-adaptation, no archive diversity, freeze-only, and no motifs/macros). Each run contains 20 tasks per rung (100 total); its Slurm defaults are 24 hours, 16 CPUs, 64 GiB, and one GPU. The runner validates inherited configs, isolates each arm/seed library, resumes only when the saved effective hash and cold-library path match, refuses nonempty orphan libraries, serializes each device\textquotesingle s assigned runs through a per-GPU queue, emits Slurm arrays, and aggregates overall and per-rung means with 95\% Student-t confidence intervals for search/report performance, transfer and mechanism-use rates, cost, resource declines, robustness, complexity, admissions, and library growth. These are executable protocols only; no full-cluster or ablation outcome is claimed.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{8 Implementation and Performance}\label{8-implementation-and-performance}
...
\section{10 Limitations and Open Problems}\label{10-limitations-and-open-problems}

\textbf{Statistical rigor.} Every reported number in Section 7 is seed-0 on one machine. The gate experiments were pre-registered and single-flip, but n = 1 per arm, attempts within a run are accumulated assaults (not independent samples), and the cross-generation two-spirals table spans evolving configurations. The repository now code-verifies the 45-run ablation suite and its confidence-interval aggregation, but those runs and the three-seed full-cluster campaign remain pending; tooling is not evidence.
...
Bai, S., Kolter, J. Z., and Koltun, V. (2019). Deep equilibrium models. \emph{NeurIPS 2019}. arXiv:1909.01377.

Banino, A., Balaguer, J., and Blundell, C. (2021). PonderNet: Learning to ponder. arXiv:2107.05407.

Barrett, D. G. T., Hill, F., Santoro, A., Morcos, A. S., and Lillicrap, T. (2018). Measuring abstract reasoning in neural networks. \emph{ICML 2018}. arXiv:1807.04225.

Bengio, Y., Louradour, J., Collobert, R., and Weston, J. (2009). Curriculum learning. \emph{ICML 2009}. doi:10.1145/1553374.1553380.

Beyer, H.-G., and Schwefel, H.-P. (2002). Evolution strategies: A comprehensive introduction. \emph{Natural Computing}, 1(1):3-52.
...
Brant, J. C., and Stanley, K. O. (2017). Minimal criterion coevolution: A new approach to open-ended search. \emph{GECCO 2017}. doi:10.1145/3071178.3071186.
...
Clune, J., Mouret, J.-B., and Lipson, H. (2013). The evolutionary origins of modularity. \emph{Proceedings of the Royal Society B}, 280(1755). arXiv:1207.2743.

Deb, K., Pratap, A., Agarwal, S., and Meyarivan, T. (2002). A fast and elitist multiobjective genetic algorithm: NSGA-II. \emph{IEEE Transactions on Evolutionary Computation}, 6(2):182-197.

Dehghani, M., Gouws, S., Vinyals, O., Uszkoreit, J., and Kaiser, L. (2019). Universal Transformers. \emph{ICLR 2019}. arXiv:1807.03819.
...
Elsken, T., Metzen, J. H., and Hutter, F. (2019a). Efficient multi-objective neural architecture search via Lamarckian evolution. \emph{ICLR 2019}. arXiv:1804.09081.

Elsken, T., Metzen, J. H., and Hutter, F. (2019b). Neural architecture search: A survey. \emph{JMLR}, 20(55):1-21. arXiv:1808.05377.

Fahlman, S. E., and Lebiere, C. (1990). The cascade-correlation learning architecture. \emph{NIPS 2 (1989)}, 524-532.
...
Fedus, W., Zoph, B., and Shazeer, N. (2022). Switch Transformers: Scaling to trillion parameter models with simple and efficient sparsity. \emph{JMLR}, 23(120). arXiv:2101.03961.
...
French, R. M. (1999). Catastrophic forgetting in connectionist networks. \emph{Trends in Cognitive Sciences}, 3(4):128-135.

Gaier, A., and Ha, D. (2019). Weight agnostic neural networks. \emph{NeurIPS 2019}. arXiv:1906.04358.

Gomez, F., and Miikkulainen, R. (1997). Incremental evolution of complex general behavior. \emph{Adaptive Behavior}, 5(3-4):317-342.

Gomez, F., Schmidhuber, J., and Miikkulainen, R. (2008). Accelerated neural evolution through cooperatively coevolved synapses. \emph{JMLR}, 9:937-965.

Grand, G., Wong, L., Bowers, M., Olausson, T. X., Liu, M., Tenenbaum, J. B., and Andreas, J. (2024). LILO: Learning interpretable libraries by compressing and documenting code. \emph{ICLR 2024}. arXiv:2310.19791.

Graves, A. (2016). Adaptive computation time for recurrent neural networks. arXiv:1603.08983.

Hansen, N., and Ostermeier, A. (2001). Completely derandomized self-adaptation in evolution strategies. \emph{Evolutionary Computation}, 9(2):159-195.

Hinton, G. E., and Nowlan, S. J. (1987). How learning can guide evolution. \emph{Complex Systems}, 1(3):495-502.

Hu, S., Lu, C., and Clune, J. (2025). Automated design of agentic systems. \emph{ICLR 2025}. arXiv:2408.08435.

Huang, B., Cheng, R., Li, Z., Jin, Y., and Tan, K. C. (2024). EvoX: A distributed GPU-accelerated framework for scalable evolutionary computation. \emph{IEEE Transactions on Evolutionary Computation}. arXiv:2301.12457.
...
Kashtan, N., and Alon, U. (2005). Spontaneous evolution of modularity and network motifs. \emph{PNAS}, 102(39):13773-13778.

Kingma, D. P., and Ba, J. (2015). Adam: A method for stochastic optimization. \emph{ICLR 2015}. arXiv:1412.6980.

Kirkpatrick, J., Pascanu, R., Rabinowitz, N., Veness, J., Desjardins, G., Rusu, A. A., Milan, K., Quan, J., Ramalho, T., Grabska-Barwinska, A., Hassabis, D., Clopath, C., Kumaran, D., and Hadsell, R. (2017). Overcoming catastrophic forgetting in neural networks. \emph{PNAS}, 114(13):3521-3526.

Krizhevsky, A. (2009). Learning multiple layers of features from tiny images. Technical report, University of Toronto.

Lang, K. J., and Witbrock, M. J. (1988). Learning to tell two spirals apart. \emph{Proceedings of the 1988 Connectionist Models Summer School}, 52-59.

Lange, R. T. (2023). evosax: JAX-based evolution strategies. \emph{GECCO 2023 Companion}. arXiv:2212.04180.

LeCun, Y., Bottou, L., Bengio, Y., and Haffner, P. (1998). Gradient-based learning applied to document recognition. \emph{Proceedings of the IEEE}, 86(11):2278-2324.
...
Li, X., Zhou, Y., Wu, T., Socher, R., and Xiong, C. (2019). Learn to grow: A continual structure learning framework for overcoming catastrophic forgetting. \emph{ICML 2019}. arXiv:1904.00310.
...
Milo, R., Shen-Orr, S., Itzkovitz, S., Kashtan, N., Chklovskii, D., and Alon, U. (2002). Network motifs: Simple building blocks of complex networks. \emph{Science}, 298(5594):824-827.
...
Ororbia, A., ElSaid, A., and Desell, T. (2019). Investigating recurrent neural network memory structures using neuro-evolution. \emph{GECCO 2019}. doi:10.1145/3321707.3321795.

Parisi, G. I., Kemker, R., Part, J. L., Kanan, C., and Wermter, S. (2019). Continual lifelong learning with neural networks: A review. \emph{Neural Networks}, 113:54-71.

Pham, H., Guan, M. Y., Zoph, B., Le, Q. V., and Dean, J. (2018). Efficient neural architecture search via parameter sharing. \emph{ICML 2018}. arXiv:1802.03268.

Pourcel, J., Colas, C., and Oudeyer, P.-Y. (2025). Self-improving language models for evolutionary program synthesis: A case study on ARC-AGI. \emph{ICML 2025}. arXiv:2507.14172.
...
Risi, S., and Stanley, K. O. (2012). An enhanced hypercube-based encoding for evolving the placement, density, and connectivity of neurons. \emph{Artificial Life}, 18(4):331-363.

Romera-Paredes, B., Barekatain, M., Novikov, A., Balog, M., Kumar, M. P., Dupont, E., Ruiz, F. J. R., Ellenberg, J. S., Wang, P., Fawzi, O., Kohli, P., and Fawzi, A. (2024). Mathematical discoveries from program search with large language models. \emph{Nature}, 625:468-475.

Rosenbaum, C., Klinger, T., and Riemer, M. (2018). Routing networks: Adaptive selection of non-linear functions for multi-task learning. \emph{ICLR 2018}. arXiv:1711.01239.
...
Stengel-Eskin, E., Prasad, A., and Bansal, M. (2024). ReGAL: Refactoring programs to discover generalizable abstractions. \emph{ICML 2024}. arXiv:2401.16467.

Tang, Y., Tian, Y., and Ha, D. (2022). EvoJAX: Hardware-accelerated neuroevolution. \emph{GECCO 2022 Companion}. arXiv:2202.05008.
...
Wang, L., Zhao, M., Liu, E., Sun, K., and Cheng, R. (2024). Tensorized neuroevolution of augmenting topologies for GPU acceleration. \emph{GECCO 2024}. arXiv:2404.01817.
...
Wernicke, S. (2006). Efficient detection of network motifs. \emph{IEEE/ACM Transactions on Computational Biology and Bioinformatics}, 3(4):347-359.
...
Woolley, B. G., and Stanley, K. O. (2011). On the deleterious effects of a priori objectives on evolution and representation. \emph{GECCO 2011}. doi:10.1145/2001576.2001707.

Xiao, H., Rasul, K., and Vollgraf, R. (2017). Fashion-MNIST: A novel image dataset for benchmarking machine learning algorithms. arXiv:1708.07747.

Ying, C., Klein, A., Real, E., Christiansen, E., Murphy, K., and Hutter, F. (2019). NAS-Bench-101: Towards reproducible neural architecture search. \emph{ICML 2019}. arXiv:1902.09635.

Yoon, J., Yang, E., Lee, J., and Hwang, S. J. (2018). Lifelong learning with dynamically expandable networks. \emph{ICLR 2018}. arXiv:1708.01547.

Zhang, C., Gao, F., Jia, B., Zhu, Y., and Zhu, S.-C. (2019). RAVEN: A dataset for relational and analogical visual reasoning. \emph{CVPR 2019}. arXiv:1903.02741.
...
Zoph, B., and Le, Q. V. (2017). Neural architecture search with reinforcement learning. \emph{ICLR 2017}. arXiv:1611.01578.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Appendix A: The Operator Registries}\label{appendix-a-the-operator-registries}

Every stage of the loop is a registry; the shipping configuration selects one operator per stage. This table is the system\textquotesingle s extension surface: adding a behavior means registering one function and naming it in config.
...
Registry
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Registered operators (shipping selection in bold)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
loop & LOOP & \textbf{hierarchical} \\
evolve strategy & EVOLVE\_STRATEGY & \textbf{routed}, \textbf{grammar}, \textbf{direct}, \textbf{composition} (ladder order) \\
init & INIT & minimal, factored, sparse, \textbf{cppn} (generative) \\
selection & SELECTION & tournament, truncation, \textbf{nsga2} \\
crossover & CROSSOVER & none, \textbf{neat} \\
mutation & MUTATION & 23 operators, 16 selected (Section 5.2) \\
train & TRAIN & none, \textbf{gradient} (composition/module), gradient\_refine, \textbf{gradient\_scheduled} (direct) \\
population train & TRAIN\_POPULATION & gradient\_batched, gradient\_refine, gradient\_scheduled (calibration-selected) \\
evaluate & EVALUATE & standard, weight\_samples, \textbf{hybrid}, augmented\_vote \\
fitness & FITNESS & 17 components, weighted blend + optional Pareto objective vector \\
speciation & SPECIATION & none, \textbf{neat} (auto-adjusting threshold) \\
schedule & SCHEDULE & random, round\_robin, \textbf{interleave\_rungs} \\
comp mutation / crossover & COMP\_MUTATION / COMP\_CROSSOVER & 5 comp operators / none, \textbf{comp\_neat} \\
decompose & DECOMPOSE & \textbf{output\_slices}, \textbf{input\_subsets}, \textbf{time\_windows}, \textbf{spatial\_patches} \\
library admission & LIBRARY\_ADMISSION & accept\_all, default, \textbf{archive} \\
\end{longtable}

\section{Appendix B: Key Hyperparameters (shipping configuration)}\label{appendix-b-key-hyperparameters-shipping-configuration}
...
\bottomrule\noalign{}
\endlastfoot
Protocol & blind support-only search; dense support accuracy for progress; support task-exact \textgreater= 0.95 on grids, support accuracy elsewhere; one held-out query report \\
...
Wall ledger & on; stone floor 0.45; structured gain over constant/copy baseline \textgreater= 0; 1 stone seeded per attempt \\
Library & archive admission; metric floor 0.9; 2 per niche; 12 per signature; GC on \\
Routed & d\_model 64, top-k 2, max 4 steps, adapter rank 4, 200 Adam steps at lr 3e-3, load-balance 0.01, 8-task replay buffer (one task replayed every 4th step), halting + edge bias on, distill required (usage floor 0.1, max 6 nodes) \\
Grammar & motif sizes 3-4 (modules), 2-4 (compositions); \textgreater=2 lineage roots; 12 productions; 3 candidates/production; 2 mutation steps \\
...
\emph{Gate-experiment variants (Section 7.3), per arm. G1: NSGA-II + objectives {[}support\_accuracy, novelty, connection\_cost{]} + novelty table on. Probe (G2): G1\textquotesingle s flips with the objective vector reduced to {[}support\_accuracy, novelty{]}, plus gradient\_scheduled (lr 0.01, 250 steps), cppn init (hidden 16, density 0.8, sin/gaussian-seeded), self-adaptive rates, the remove\_connection/remove\_hidden\_node operators, parsimony band 0.01, zeroed hidden/complexity penalties, stall window 60, and a 600 s per-attempt time budget.}

\section{Appendix C: Reproducibility}\label{appendix-c-reproducibility}

Every number in Sections 7 and 8 traces to an immutable archived artifact under \texttt{ai/archive/\textless{}tag\textgreater{}/}. \path{paper/evidence/manifest.json} names every claim and figure input and pins its SHA256; the paper build fails on a missing or changed artifact. Configs may recursively \texttt{extend} one path or an ordered path list, resolved relative to the declaring file; mappings deep-merge left-to-right and then into the child, while scalars and lists replace. New runs preserve the leaf-file digest, the ordered source paths and digests, and the canonical merged digest, then snapshot both the source TOML and the complete CLI-adjusted effective configuration with independent digests. They also record a hardware/runtime profile and selected execution mode. Implicit resume loads the effective snapshot. Rolling checkpoints and run summaries are written before task zero and after every task. A \texttt{fresh\_per\_task} control freezes the starting library once, solves every task against a disposable copy with the shared worker pool detached, and discards all task-local keys, providing a true no-memory baseline without configuration duplication.
...
\bottomrule\noalign{}
\endlastfoot
Headline 400-task run (S7.2) & \path{orchestrated_overmind.toml}, schedule extended to rungs 1-6 & 0 & cold (empty) \\
Stone-seeded snapshot (S7.2) & \path{orchestrated_overmind.toml}, rungs 1-5 & 0 & one entry (the probe\textquotesingle s two-spirals stone) \\
G0 baseline (S7.3) & \path{diag_g0_two_spirals.toml} & 0 & cold \\
G1 divergent selection (S7.3) & \path{diag_g1_two_spirals.toml} & 0 & cold \\
G1 warm continuation (S7.3) & \path{diag_g1_two_spirals.toml} & 0 & G1\textquotesingle s surviving library \\
Flip-to-GO probe (S7.3) & \path{diag_g2_two_spirals.toml} & 0 & cold \\
Full-ladder coverage run (S10) & \path{smoke_all_rungs.toml} & 0 & cold \\
Full-ladder canary (S7.5) & frozen \path{20260712_145535_orchestrated/config.toml} & 0 & cold; curriculum-interleaved \\
\end{longtable}

The following protocols are checked into the repository but \textbf{all outcomes remain pending}:
...
\bottomrule\noalign{}
\endlastfoot
Full-cluster flagship & \texttt{full\_cluster.toml} / \path{campaigns/full_cluster.toml} & 0, 1, 2 & cold library; 18 rungs x 20 tasks; depth budgets 2,000, 1,000, 500, 250, 125, 62, 31, 15, and 7; 7,200 s local and 21,600 s cumulative caps \\
Paper-core ablations & \path{ablations/manifest.toml} & five seeds for 6 P0 arms; three for 5 P1 arms & 45 isolated runs; rungs 1-5 x 20 tasks; mean and 95\% Student-t CI aggregation \\
\end{longtable}

All reported configurations are preserved with the frozen evidence. Historical runs used one Apple M4 Max (128 GB unified memory), CPU-resident search with a 12-worker assessment pool, PyTorch 2.12.0, and Python 3.12. The canonical manuscript is \texttt{paper/preprint.md}; a pinned Pandoc conversion and authenticated official NeurIPS 2026 style generate the preprint, submission, and final variants. Submission and final builds fail closed on unresolved checklist answers, missing evidence, template drift, or overfull boxes. The code-release license and artifact-deposit location remain submission blockers.

\section{Appendix D: Compute and Broader Impact}\label{appendix-d-compute-and-broader-impact}

\textbf{Compute.} The experiments reported in Section 7 total approximately 20.0 hours of recorded task wall clock on one machine: 6.3 hours across the previously reported lower-ladder/gate arms plus 13.70 hours (49,311.5 s) for the 180-task canary. The canary ran 3,987 generations; the earlier 6.3-hour subtotal includes 2.83 h for the headline run (10,198 s over 400 tasks), 0.05 h for G0, 0.10 h for G1 cold, 1.13 h for the G1 warm continuation, 1.64 h for the interrupted probe, and 0.56 h for the stone-seeded snapshot. The per-task \texttt{seconds} field makes the accounting reproducible. All reported computation was CPU-resident on one consumer machine; no GPUs or cloud compute were used.
...
\section{Appendix E: Full-Ladder Canary by Rung}\label{appendix-e-full-ladder-canary-by-rung}

Held-out query accuracy is primary; support accuracy is shown separately. \texttt{N/A} means no held-out value was recorded and is not imputed as zero.
