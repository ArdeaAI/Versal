...
% Manuscript SHA256: 2bed9dba8f61e4ec7cfd3d21d61855f10473cfd23ebee21c8023e625de9f0866
...
14 & \texttt{psicov} & 58 & CONTINUOUS 271k to 5.1M -\textgreater{} CONTINUOUS 4.8k to 89k & NAS-Bench-360; widths vary per task \\
...
Task counts and I/O widths were enumerated live from the published dataset on 2026-07-06 (the probe transcript is archived with the frozen artifacts). Widths are the flattened Level0 view that library signatures key on; temporal rungs additionally unroll the TIME axis under the stepped substrate. Rung difficulty is intrinsic to the data, not to any code path: higher rungs mostly mean wider flattened I/O and harder generalization, not new mechanisms. Live probes measure rung 11 (\texttt{spherical}) at 10,800 inputs by 100 outputs, rung 12 (\texttt{cosmic}) at 65,536 by 65,536, rung 13 (\texttt{darcy\_flow}) at 177,241 by 177,241, and rung 14 (\texttt{psicov}) as per-task variable, spanning 271,377 to 5,095,857 inputs and 4,761 to 89,401 outputs across probed tasks. These rungs currently exceed the minimal-initialization regime (Section 10).
...
\textbf{Statistical rigor.} Every number in Section 7 is seed-0 on one machine. The gate experiments were pre-registered and single-flip, but n = 1 per arm, attempts within a run are accumulated assaults (not independent samples), and the cross-generation two-spirals table spans evolving configurations. Seed sweeps, confidence intervals, and a frozen-config replication of the whole gate ladder are the first items in \texttt{paper\_todo.md}.
