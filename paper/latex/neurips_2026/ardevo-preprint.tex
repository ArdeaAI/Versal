...
% Manuscript SHA256: 43242cb30fc3e3314c405e4ca0ab1b5dabba287786dfcc0d4da1d96839c86e84
...
We are explicit about status: the reported campaigns exercise rungs 1 to 6 end to end, with rungs 3 and 6 still unsolved under the orchestrated regime. A coverage run completes bounded attempts on every rung 1 to 18 under the wall levers, with one init-wall rung solved (Section 10). The current implementation now provides blind support-only search, exact-grid ARC reporting, output-shape inference, baseline and coverage metrics, bounded adaptive decomposition, and a no-memory control, but no new ARC solve is claimed. The companion document \texttt{paper\_todo.md} enumerates the ablation and scaling program required before submission.
...
\emph{Figure 6: Motif census atlas over the completed run\textquotesingle s frozen library: recurring size-3/4 substructures mined by exact permutation-canonical fingerprinting, grouped by diversity class (gated, gated-plus-macro, macro, mixed, uniform) and ranked by support (the number of distinct entries containing the motif).}

\subsection{7.5 Pending full-cluster and ablation protocol}\label{75-pending-full-cluster-and-ablation-protocol}

The repository now contains a code-verified scaling protocol whose outcomes are \textbf{pending and are not results in this paper}. \path{configs/full_cluster.toml} recursively extends the unchanged \path{configs/preflight.toml} (which still points at \texttt{library}), restores 20 tasks on each of 18 rungs (360 per run), selects \texttt{ClusterCUDA}, and explicitly selects population CUDA training (minimum 768 nodes, maximum padded size 8,192, adaptive microbatch splitting). Its recursion budgets are 2000/1000/500/250/125 generations at depths 0 through 4; each depth has a 7,200-second local cap and the entire recursive solve has a 21,600-second cumulative cap. Routed distillation permits 16 nodes; library-archive caps rise to 4 entries per niche and 48 per signature; adaptive direct/allocation guards use 55\% of available host memory less 8 GiB and 70\% of free CUDA memory less 2 GiB, further bounded by scheduler cgroups; composition glue is compact float32 and permits eight library-reference boundaries. Complete run-plus-library S3 snapshots are scheduled after every task with two retained locally. Run/library locks exclude live manual snapshots, final completion requires a durable \texttt{done} receipt, and restore verifies all hashes before atomically installing the recovered tree. The config\textquotesingle s campaign table is metadata; \path{configs/campaigns/full_cluster.toml} materializes isolated cold-library runs for seeds 0, 1, and 2 with one GPU, 16 CPUs, 128 GiB, and a 48-hour job cap each, with a Slurm-array concurrency default of three. A 48-hour allocation may not finish a run; resubmitting the same suite resumes the hash-matched checkpoint and cold-library lineage.
...
\textbf{The upper ladder remains an evidence gap.} Production campaigns cover rungs 1 to 6, and rung 6 is unsolved. The archived one-task-per-rung coverage run completed bounded attempts on all 18 rungs; cosmic solved through decomposition at 0.986 and fsd50k at 0.955, while darcy\_flow, psicov, PGM, and ARC failed with complete forensics rather than wedging. Coverage at probe budgets is not a solve campaign. The current code removes several methodological blockers that the archived run exposed: categorical-logit-aware preflight guards, adaptive bounded output slices, shared-canvas variable-grid evaluation, support-only search, exact shape-and-grid reporting, trivial baselines, baseline-relative wall admission, generic tied spatial motifs with an evolvable untie operation, and a discovered graph grammar. These mechanisms have regression tests but no campaign result in this paper. ARC-AGI therefore remains an open result, and ARC-AGI-3 still lacks an interactive harness.
