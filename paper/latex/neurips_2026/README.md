...
- `versal-preprint.tex` and `.pdf` are generated from `paper/preprint.md`.
- `versal-technical-report.tex` and `.pdf` combine that core with `paper/technical_appendix.md` before the shared references.
...
- `versal-submission.*` and `versal-final.*` are generated on demand and should not be hand-edited.
