...
% Manuscript SHA256: d7fc792cded1e8964c645fdfbd86322701fa539f2150abbc4063cdcb26673490
...
\urlstyle{same}
\hypersetup{
  pdftitle={Versal: Toward Task-General Intelligence with Persistent, Compounding Neuroevolution},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}
\setcounter{secnumdepth}{-\maxdimen}

\title{Versal: Toward Task-General Intelligence with Persistent, Compounding Neuroevolution}
\author{%
  John Gardner \\
  Ardea AI \\
  \texttt{john@ardea.io}
...
\section{4 The Current Versal Method}\label{4-the-current-versal-method}
...
\emph{Figure 1: The Versal task loop. Search begins from persistent memory, escalates through reuse and structural search, may recurse over typed subtasks, and returns verified artifacts to a lifecycle-managed library. Held-out query evaluation occurs only after search selects an executable artifact.}
...
\pandocbounded{\includegraphics[keepaspectratio]{../../figures/fig7_canary.png}}
...
\textbf{No matched external baselines.} Versal has not yet been compared under equal budgets with a fixed MLP, canonical NEAT, WANN-style search, or random topology search on this contract. The current evidence supports internal behavior claims, not superiority.
...
The dataset is generated in a separate repository and published as \path{Ardea/Icarus-dataset}. Versal vendors only the loader/encoder runtime. Rungs 4 and 5 originate from policy rollouts but are stored and evaluated as supervised traces. No interactive environment is present in the consumer. The rung scheduler is checkpointable and normally interleaves rungs so that library state changes between families.
...
\pandocbounded{\includegraphics[keepaspectratio]{../../figures/fig5a_overmind.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{../../figures/fig5e_artifact_triptych.png}}
