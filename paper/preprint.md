...
The repository now contains a code-verified scaling protocol whose outcomes are **pending and are not results in this paper**. `configs/full_cluster.toml` recursively extends its pinned preflight base, restores 20 tasks on each of 18 rungs (360 per run), selects `ClusterCUDA`, and explicitly selects population CUDA training (minimum 768 nodes, maximum padded size 8,192, adaptive microbatch splitting). Its recursion budgets are 2000/1000/500/250/125 generations at depths 0 through 4; each depth has a 7,200-second local cap and the entire recursive solve has a 21,600-second cumulative cap. Routed distillation permits 16 nodes; library-archive caps rise to 4 entries per niche and 48 per signature; adaptive direct/allocation guards use 55% of available host memory less 8 GiB and 70% of free CUDA memory less 2 GiB, further bounded by scheduler cgroups; composition glue is compact float32 and permits eight library-reference boundaries. Content-addressed snapshots are scheduled after every task with two retained locally. Run/library locks exclude live manual snapshots, final completion requires a durable `done` receipt, and restore verifies all hashes before atomically installing the recovered tree. The config's campaign table is metadata; `configs/campaigns/full_cluster.toml` materializes isolated cold-library runs for seeds 0, 1, and 2 with one GPU, 16 CPUs, 128 GiB, and a 48-hour job cap each, with a Slurm-array concurrency default of three. A 48-hour allocation may not finish a run; resubmitting the same suite resumes the hash-matched checkpoint and cold-library lineage.
...
|---|---|---|---|
| Full-cluster flagship | `full_cluster.toml` / `campaigns/full_cluster.toml` | 0, 1, 2 | cold library; 18 rungs x 20 tasks; depth budgets 2,000, 1,000, 500, 250, and 125; 7,200 s local and 21,600 s cumulative caps |
