...
*Working draft, revised 2026-07-13. The method and reproducibility pipeline are current; `paper_todo.md` enumerates the run-gated evidence still required for submission.*
...
We are explicit about status: the lower-ladder campaigns exercise rungs 1 to 6 end to end, with rungs 3 and 6 still unsolved under the orchestrated regime. A later 180-task canary completed ten tasks on every rung (Section 7.5), but its upper-rung support fits often failed to generalize. On ARC, mean held-out cell accuracy was 0.5128 (maximum 0.84375); this is not task-exact ARC evidence and no ARC solve is claimed. The canary predates the engineering remedies it motivated, so those fixes are method claims backed by tests, not retroactive explanations of measured outcomes. The companion document `paper_todo.md` enumerates the remaining ablation and scaling program.
...
**Composition (hierarchical reuse).** The composition strategy evolves a per-task population of **composition genomes**: small typed DAGs whose nodes reference either live module species from a shared pool or frozen library entries, wired by trainable linear glue (rank-factorized above a size threshold). Before constructing a population or routed distillation, it computes the exact dense or factorized representation. Post-canary decomposition skeletons instead preserve immutable axis/shape-derived port maps as compact index runs and execute them by gather/scatter for output slices, input subsets, time windows, and spatial patches; these maps survive serialization, crossover, mutation, writeback, nesting, and resource accounting. The smoke profile retains a five-million-value guard; canary, brute, and preflight use unrestricted float32 glue on the 128-GiB workstation, while the CUDA canary and full-cluster profiles add stage-aware adaptive host/device envelopes. The strategy re-verifies its champion against current module state before acceptance: stale weights or stale metrics can never be persisted.
...
Five controls bound task cost: the generation budget ladder, the stall detector, an optional per-depth wall-clock budget (`max_task_seconds`), an optional cumulative top-level budget inherited by every recursive sub-solve (`max_total_task_seconds`), and dimension-derived pre-allocation caps on flat initialization and composition glue. Each recursive depth receives a fresh local deadline but cannot outlive the earlier cumulative deadline. Both clocks are soft: an in-flight generation finishes and may still produce an accepted result. Once cumulative expiry is observed, however, the orchestrator starts no new recursive sub-solve, solvability probe, refinement, decomposition operator, or parent retry. The allocation caps act earlier, before a generation boundary exists. Every attempt serializes an `Attempt` row: outcome (`library_hit`, `refined`, `evolved`, `decomposed`, `failed`), metric, generations, winning or best-losing strategy, failure stage, refinement generations, wall seconds, per-stage seconds, champion size metrics, and the structure-versus-weights sample metrics of Section 7.3 when the evaluator emits them.
...
- **Macro genes.** A macro embeds an entire library entry as one frozen unit with one innovation marker: an atomic crossover and speciation unit, resolved at decode time against the live library, with hidden identity stubs for its outputs that every mutation operator is forbidden to retarget. Macros are how the search reuses a discovered network wholesale, the analogue of dropping in a hand-designed cell, except nothing about the cell was hand-designed. All module and composition expansion obeys one configurable reference-depth invariant: the selected root payload is depth 0 and free, while each `library:<key>` dereference across either kind of boundary consumes exactly one level. `[evolution.composition] max_inline_depth` defaults to 4 in code and in the smoke overlay; the canary methodology and its preflight, brute, lattice, and full-cluster derivatives use 8. Cycles are rejected independently of this cap.
...
The structural fingerprint keeps node identifiers verbatim, so it cannot recognize that two entries grew the same four-node gadget under different numbering. The **motif census** fills that gap: each payload becomes a labeled dataflow graph, connected k-subgraphs up to k = 5 are enumerated exactly by ESU with deterministic truncation, and each subgraph reduces to an exact permutation-canonical form. This raw frequency census is useful for grammar induction and descriptive inventory; its atlas is **not evidence of architectural discovery**. The post-canary discovery layer excludes pure input/output plumbing, locks candidates from support/library evidence before reading query outcomes, and ranks them against 64 deterministic label- and degree-preserving null graphs using independent lineage support, within-signature performance percentile, robustness percentile, and reuse percentile. `Observed` requires surprise z >= 2 and positive performance association; `functional` additionally requires a frozen-edge knockout loss of at least 0.02 that is worse than 95% of 16 matched controls after at most 25 recovery steps; `replicated` requires functional evidence in at least two lineage roots. No candidate is called a new cell, convolution, or architectural invention without replicated evidence. Motif counting follows Milo et al. (2002); the enumerator is Wernicke's ESU (Wernicke, 2006).
...
The reported experimental program has three arms: lower-ladder compounding, the controlled two-spirals case study, and a completed 180-task canary spanning all 18 rungs. All are single-seed (seed 0) on one Apple M4 Max with PyTorch 2.12 and CPU-resident search. Section 7.6 separately records the code-verified multi-seed protocols whose outcomes remain pending. We report every completed, interrupted, and failed run represented in the frozen evidence.
...
*Figure 6: Descriptive motif-frequency atlas over the completed lower-ladder run's frozen library. It is an inventory and grammar-induction input, not evidence that the system discovered a functional architectural primitive.*

### 7.5 Full-ladder canary

The frozen run `20260712_145535_orchestrated` completed all 180 scheduled tasks (ten per rung) in 49,311.5 recorded task-seconds and 3,987 generations. Outcomes were 74 evolved, 64 failed, 36 library hits, 4 refinements, and 2 decompositions. Held-out values were recorded for 137 tasks. The library admitted 84 entries during the run and retained 69 after garbage collection. Appendix E gives the complete rung table; all aggregates use held-out query accuracy as primary and show support accuracy separately.

Generalization was strong on rungs 1, 2, 4, 12, and 15: mean held-out accuracies were 1.0000, 0.9846, 0.9525, 0.9840, and 0.9846, respectively. This did not extend uniformly upward. Fashion-MNIST/MNIST support averaged 0.9838 while query averaged 0.5100; CIFAR support averaged 0.7350 while query averaged 0.1531; PGM support was 1.0000 while query averaged 0.1077. ARC support averaged 0.9994, but held-out **cell-level** accuracy averaged 0.5128 and peaked at 0.84375. The run did not contemporaneously record exact-grid ARC scores, so these rows cannot support an ARC task solve; high background-dominated cell accuracy would not imply one in any case.

The 48 deadline-counter events concentrated on upper-rung failures and show the intended bounded behavior, while also reminding us that a deadline sampled between generations cannot interrupt one expensive generation. The library curve nevertheless compounded from zero to 84 admissions, and later ARC tasks increasingly reached routed/composed artifacts. The router transcript is diagnostic: adapter-space scores often reached 1.0, while distilled compositions ranged from 0 to near 1.0; below-bar or failed distillations escalated rather than counting as solutions. We interpret this as correct escalation plus a missing generic parent-wiring capability, not an ARC-specific router failure. The persisted router state reached 5.3 GiB, dominated by wide adapters and heads.

The cautious impact claim is therefore narrow but meaningful: one persistent evolutionary system executed across all 18 rungs and multiple modalities without a rung-specific search loop. The same run shows that support fitting and reusable structure have not produced reliable upper-rung generalization. It is one seed under one curriculum, predates the compact-port and routed-handoff changes, lacks contemporaneous exact-grid and matched-baseline metrics, and pins configuration bytes but no executable Git revision.

All remedies motivated by the canary are **post-canary method changes**, not measured outcomes: compact fixed port maps replace dense decomposition glue; executable below-bar routed distillations become deduplicated composition seeds with strategy metrics; router format v2 keeps the complete gate embedding table resident but decodes selected experts and loads wide adapter/head shards on demand, adds activated parameters to the optimizer, and evicts only after a durable between-task checkpoint; archive format v2 deduplicates content by hash; and motif discovery now requires null and counterfactual evidence. A matched rerun is required to measure any effect.

### 7.6 Pending full-cluster and ablation protocol

The repository now contains a code-verified scaling protocol whose outcomes are **pending and are not results in this paper**. `configs/canary.toml` is the single full-method definition; `configs/full_cluster.toml` extends it with 20 tasks on each of 18 rungs (360 per run), selects `ClusterCUDA`, and explicitly selects population CUDA training (minimum 768 nodes, maximum padded size 8,192, adaptive microbatch splitting). Its depth-0-through-8 recursion budgets are 2000/1000/500/250/125/62/31/15/7 generations; each attempt has a 7,200-second cap and the entire recursive solve has a 21,600-second cumulative cap. Routed distillation permits 16 nodes; library-archive caps rise to 4 entries per niche and 48 per signature; adaptive direct/allocation guards use 55% of available host memory less 8 GiB and 70% of free CUDA memory less 2 GiB, further bounded by scheduler cgroups; composition glue is compact float32 and permits eight library-reference boundaries. Content-addressed snapshots are scheduled after every task with two retained locally. Run/library locks exclude live manual snapshots, final completion requires a durable `done` receipt, and restore verifies all hashes before atomically installing the recovered tree. The config's campaign table is metadata; `configs/campaigns/full_cluster.toml` materializes isolated cold-library runs for seeds 0, 1, and 2 with one GPU, 16 CPUs, 128 GiB, and a 48-hour job cap each, with a Slurm-array concurrency default of three. A 48-hour allocation may not finish a run; resubmitting the same suite resumes the hash-matched checkpoint and cold-library lineage.

The paper-core ablation manifest contains 45 planned runs over rungs 1 through 5: six P0 arms at five seeds each (full, no routing, no decomposition, no hierarchy, no refinement, and no library reuse) plus five P1 arms at three seeds each (no curriculum, no self-adaptation, no archive diversity, freeze-only, and no motifs/macros). Each run contains 20 tasks per rung (100 total); its Slurm defaults are 24 hours, 16 CPUs, 64 GiB, and one GPU. The runner validates inherited configs, isolates each arm/seed library, resumes only when the saved effective hash and cold-library path match, refuses nonempty orphan libraries, serializes each device's assigned runs through a per-GPU queue, emits Slurm arrays, and aggregates overall and per-rung means with 95% Student-t confidence intervals for search/report performance, transfer and mechanism-use rates, cost, resource declines, robustness, complexity, admissions, and library growth. These are executable protocols only; no full-cluster or ablation outcome is claimed.
...
**Pinned and calibrated performance work.** Performance refactors must reproduce the reference implementation: the compact-column substrate is bitwise-equal to the dense layout for forward and feedforward training (recurrent training differs by one ulp from a documented reassociation), and mutation rewrites are compared gene-for-gene against frozen reference bodies. Historical M4 Max measurements reduced per-candidate width-784 training from 399.6 to 30.0 ms and removed a geometry-mutation wall. The population trainer covers the scheduled optimizer, tracks microbatch and OOM fallback statistics, and is normally selected through a persisted hardware/runtime fingerprint only after numerical validation and a 15% measured speedup over the actual process-pool path. The pending full-cluster profile instead makes the intended CUDA path explicit (`ClusterCUDA`, `batched = true`): batchable populations execute as one CUDA tensor program, while small or non-batchable candidates use the CPU pool and allocation failures deterministically shrink the microbatch before candidate-wise fallback. `assess_workers = "auto"` takes the minimum of logical CPUs, process affinity, and scheduler allocations (`SLURM_CPUS_PER_TASK`, `PBS_NP`, or `NSLOTS`), then reserves four CPUs for the main process, OS, and device feeder. This preserves the method while making hardware use a declared protocol. Each run summary records CPU, memory, PyTorch, Metal/CUDA availability, thread counts, selected execution mode, and worker count. The CUDA profile is code-verified but has no campaign result.

**Configuration and resource provenance.** A top-level `extends` path or ordered path list is resolved recursively relative to the declaring TOML; parent mappings merge left-to-right, child mappings overlay them recursively, and child scalars/lists replace inherited values. Cycles and missing parents fail with their source chain. Runs retain the leaf source digest, an ordered digest list for every inherited source, and a canonical effective-config digest; they also snapshot the source TOML and complete CLI-adjusted effective JSON, which implicit resume reloads. The adaptive resource policy estimates each stage's actual residency before allocation. Host limits take the lesser of the configured fraction of cgroup-bounded total RAM and currently available/cgroup-remaining RAM, then subtract the host reserve; CUDA limits similarly take the lesser of the configured VRAM fraction and currently free VRAM, then subtract the device reserve. Slurm memory allocations are honored through their cgroup. Host cost is glue values times representation bytes (32 for legacy Python tuples or 4 for compact float32) times resident population; training cost uses 20 bytes per value times concurrent trainers. Composition populations, routed distillation, and decomposition skeletons are assembled/trained on CPU and therefore charge the unified host envelope. Direct populations charge their actual CPU, Metal, or CUDA device and actual population/concurrency mode. Fixed positive value caps retain their historical semantics and take precedence. These checks are deterministic estimates, not reservations or guarantees against allocator failure.

**Forensic observability.** The trial writes a one-row-per-task run summary and a rolling resume checkpoint after every task, including crash paths, and every attempt row carries wall seconds, per-stage seconds, champion size metrics, and the structure-versus-weights sample statistics. Four production incidents justify the design. An overnight run wedged for eight hours on CIFAR at 10% CPU; the run records attributed it to the geometry mutators' main-thread pair sweep (not the device, which was a red herring both ways), and the fix above followed with a regression guard. Weeks later, stage forensics showed composition stages consuming 849 of 860 s per task; the cause was a cycle-repair helper degraded to a per-edge constructor storm by an earlier refactor, fixed to 0.06 ms per call. A third incident (2026-07-06) exposed the budget's blind spot: a reconnaissance run wedged twice on one 409,600-input task, because the per-task wall-clock budget is sampled only between ladder stages and between generations, and an oversize population's initialization plus first generation (measured: 2.6 s and 0.79 GB per genome at that width, roughly 3 minutes and 50 GB for the population) runs for hours before either boundary exists. The remotely logged scalars (exactly one routed-strategy generation, then silence) plus two lines of arithmetic localized it without a debugger; the fix is a pre-flight guard that declines a direct attempt from its dense-init gene count alone, byte-identical when off. The same blind spot has a second cost unit: on a long-TIME-axis task, a factored-init genome's hidden latents turn one candidate's BPTT training into a twenty-minute, twenty-gigabyte unit that blocks its whole generation, which is why the historical full-ladder smoke configuration kept temporal rungs on minimal initialization and why an in-generation deadline check remains queued work. A fourth incident (2026-07-12) showed that rank factorization can remain too large in the Python genome representation: a Psicov task with 13,966,425 inputs and 245,025 outputs required 113,936,625 rank-8 glue values per minimal candidate, about 3.40 GiB, or 163 GiB for population 48. The host killed the parent process; the pool workers' broken pipes were secondary. The current smoke profile's five-million-value guard computes exact dense or factored initial glue before allocation; the full-method workstation profiles deliberately remove that fixed cap, and the CUDA profiles add adaptive resource accounting. A separate routed-distillation diagnostic counted 50,917,833 values on Psicov; that number describes one candidate pathway, not a 48-member composition population, so its stage multiplicities are one resident candidate and one concurrent trainer. Ordinary composition uses population multiplicity 48 and its active assessment concurrency. The full-cluster adaptive policy accounts for these stage differences explicitly; range-backed port columns also avoid a 0.47 GiB materialized integer list at this width. This is a representation-level resource bound, not a benchmark-specific method or a Psicov result. All four incidents were diagnosable from run artifacts or exact allocation arithmetic. The equivalence tests that pinned semantics caught none of them; only cost observability did, which is why attempt rows carry timing by construction. The diagnostic portrait follows computation rather than lineage: global and inter-network flow enters gold top-left card anchors, leaves from rendered output nodes, and nested execution connects a containing footprint node to the contained network's anchor; internal edges remain node-to-node and no redundant structural-reference overlay is drawn. Traffic order is retained across eight-card rows on a content-sized, 300-DPI canvas with extra horizontal margin for curved paths.
...
**Routing as a bet on library scale.** The lower-ladder cold run produced no routed solves, but the full-ladder canary later produced routed compositions and exposed large adapter-to-distillation gaps. That is evidence that routing and escalation execute across modalities, not that routing generalizes: many adapter-space clears failed to survive as reusable compositions. The post-canary executable handoff addresses the lost-parent case, and router v2 addresses the 5.3 GiB state footprint; neither has a matched campaign outcome.

---

## 10 Limitations and Open Problems

**Statistical rigor.** Every reported number in Section 7 is seed-0 on one machine. The gate experiments were pre-registered and single-flip, but n = 1 per arm, attempts within a run are accumulated assaults (not independent samples), and the cross-generation two-spirals table spans evolving configurations. The repository now code-verifies the 45-run ablation suite and its confidence-interval aggregation, but those runs and the three-seed full-cluster campaign remain pending; tooling is not evidence.
...
**The upper ladder remains a generalization gap.** The canary completed ten tasks per rung, but rungs 11, 13, and 14 had few or no held-out reports and no accepted outcomes, while image, PGM, and ARC rows showed severe support/query gaps. ARC has held-out cell-level evidence only, no contemporaneous exact-grid or matched-baseline result. The compact port map, routed handoff, sharded router, content-addressed archive, and evidence-ranked motif discovery all postdate the run and have regression tests but no campaign result. ARC-AGI therefore remains open, and ARC-AGI-3 still lacks an interactive harness.
...
| Module pool / compositions | 64 modules (elitism 1) / 48 compositions (elitism 2); module advance every 3rd comp generation; champion-only writeback; attribution = max with 0.95 decay; absorb top-2 on miss; glue rank 8 above 4096 elements; tuple storage; initial glue cap 5,000,000 values (0 disables); reference depth 4 |
...
Every number in Sections 7 and 8 traces to an immutable archived artifact under `ai/archive/<tag>/`. `paper/evidence/manifest.json` names every claim and figure input and pins its SHA256; the paper build fails on a missing or changed artifact. Configs may recursively `extend` one path or an ordered path list, resolved relative to the declaring file; mappings deep-merge left-to-right and then into the child, while scalars and lists replace. New runs preserve the leaf-file digest, the ordered source paths and digests, and the canonical merged digest, then snapshot both the source TOML and the complete CLI-adjusted effective configuration with independent digests. They also record a hardware/runtime profile and selected execution mode. Implicit resume loads the effective snapshot. Rolling checkpoints and run summaries are written before task zero and after every task. A `fresh_per_task` control freezes the starting library once, solves every task against a disposable copy with the shared worker pool detached, and discards all task-local keys, providing a true no-memory baseline without configuration duplication.
...
| Full-ladder canary (S7.5) | frozen `20260712_145535_orchestrated/config.toml` | 0 | cold; curriculum-interleaved |

The following protocols are checked into the repository but **all outcomes remain pending**:

| Planned campaign | Configuration / manifest | Seeds | Per-run envelope |
|---|---|---|---|
| Full-cluster flagship | `full_cluster.toml` / `campaigns/full_cluster.toml` | 0, 1, 2 | cold library; 18 rungs x 20 tasks; depth budgets 2,000, 1,000, 500, 250, 125, 62, 31, 15, and 7; 7,200 s local and 21,600 s cumulative caps |
| Paper-core ablations | `ablations/manifest.toml` | five seeds for 6 P0 arms; three for 5 P1 arms | 45 isolated runs; rungs 1-5 x 20 tasks; mean and 95% Student-t CI aggregation |
...
**Compute.** The experiments reported in Section 7 total approximately 20.0 hours of recorded task wall clock on one machine: 6.3 hours across the previously reported lower-ladder/gate arms plus 13.70 hours (49,311.5 s) for the 180-task canary. The canary ran 3,987 generations; the earlier 6.3-hour subtotal includes 2.83 h for the headline run (10,198 s over 400 tasks), 0.05 h for G0, 0.10 h for G1 cold, 1.13 h for the G1 warm continuation, 1.64 h for the interrupted probe, and 0.56 h for the stone-seeded snapshot. The per-task `seconds` field makes the accounting reproducible. All reported computation was CPU-resident on one consumer machine; no GPUs or cloud compute were used.
...
## Appendix E: Full-Ladder Canary by Rung

Held-out query accuracy is primary; support accuracy is shown separately. `N/A` means no held-out value was recorded and is not imputed as zero.
