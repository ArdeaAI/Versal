...
*Working draft, 2026-07-06. Prepared for submission; see `paper_todo.md` for the remaining experimental program.*
...
We are explicit about status: rungs 1 to 6 are exercised end to end today, with rungs 3 and 6 still unsolved under the orchestrated regime; rungs 11 to 14 are blocked by a known initialization wall (Section 10); and ARC-AGI remains a portability constraint rather than a reported result. The companion document `paper_todo.md` enumerates the ablation and scaling program required before external claims.
...
| Rung | Family | Tasks | Contract (input -> output) | Note |
|---|---|---|---|---|
...
| 4 | `pole.*` | 39 | CONTINUOUS 4 -> 1 | policy-rollout provenance, differentiable regression |
| 5 | `double_pole.*` | 39 | CONTINUOUS 6 -> 2 | as rung 4 |
| 6 | MNIST / Fashion-MNIST | | width 784 | first real image statistics |
| 7 | CIFAR | | width 3072 | the wide-image stress point |
| 8-16 | NAS-Bench-360 families | | one family per rung | ecg, satellite, ninapro, spherical, cosmic, darcy_flow, psicov, fsd50k, deepsea |
| 17 | RAVEN / PGM | | matrices | abstract relational reasoning |
| 18 | ARC-AGI | | native demo/test split | fixed split; the target regime |

Rung difficulty is intrinsic to the data, not to any code path: higher rungs mostly mean wider flattened I/O and harder generalization, not new mechanisms. Rung 11 (`spherical`) is measured at 108,300 inputs by 100 outputs; rungs 12 to 14 add output widths of 65k, 17k, and 205k respectively. These four rungs currently exceed the minimal-initialization regime (Section 10).
...
Thirteen operators are selected in the default configuration (seventeen are registered; the probe and all-features variants add the two removal operators below, for fifteen). The notable entries, beyond standard NEAT add-connection and enable/disable toggles:
...
Two omissions are load-bearing. Weight perturbation is registered but never selected when gradient training is on: random weight noise fights the optimizer and measurably stalls every task family. Canonical NEAT single-edge node splitting is likewise registered but unselected; a one-input hidden node adds no representational capacity, and assembling a useful multi-input node from it requires a coordinated sequence of connection additions the search rarely finds.

**Geometry operators.** Input nodes on grid-shaped tasks are stamped with their raw axis-index coordinates. The geometry trio then reads coordinates: `add_local_connection` weights candidate edges by exponential decay in coordinate distance, `add_local_node` grows a node whose fan-in is a coordinate neighborhood (placed at the centroid), and `add_shared_motif` replicates a template node's connectivity pattern at other coordinate neighborhoods with independent weights: a structural convolution prior with no weight tying, which is as far as the no-priors constraint permits. Coordinate distance across incomparable banks is infinite, so locality never wires a binary flag and a pixel into one receptive field. Off-grid, all three are no-ops.
...
The train stage is a registered operator like everything else. `gradient` runs Adam (Kingma and Ba, 2015) on the support loss for a configured number of steps with weight decay, then writes tuned weights back into the genome (Lamarckian writeback; Whitley et al., 1994). `gradient_refine` deep-supervises refinement genomes with a loss at every recursive pass, later passes weighted more, through full backpropagation through the recursion. `gradient_scheduled` wraps the same body in a linear-warmup, cosine-decay learning-rate envelope; Section 7.3 shows this envelope alone moves a fixed topology from 0.55 to above 0.92 query accuracy on the generator landscape, which is why it exists. A batched population trainer ports TensorNEAT's padded-tensor idea (Wang et al., 2024) natively to PyTorch: a generation of plain feedforward candidates trains as one tensor program whose folded loss makes elementwise Adam updates exactly equal to per-candidate optimizers, with non-batchable candidates (products, macros, recurrence) partitioned to a process pool. It is registered, tested, and switched off on the development machine because the 12-worker pool measurably beats it there (Section 8).
...
The compounding signature is in the cost and outcome profile, not just the solve count:
...
- **Refinement pays, then sleeps.** 46 refinement attempts on hits produced 17 strict improvements (37%, for 487 refinement generations), while 188 further attempts were skipped by the decayed cooldown: the designed steady state, in which a family that stops improving stops being paid for. Eleven superseded entries were tombstoned along the way, and garbage collection swept 14 provably unreachable tombstones at run end.
- **Reuse goes five levels deep.** The final library holds 19 level-1, 7 level-2, 4 level-3, 4 level-4, and 1 level-5 entries: compositions of compositions of compositions, grown unscripted in one cold run.
...
- **The failures are informative.** Rung 3 peaked at 0.901 and rung 6 at 0.900 against the 0.95 bar; one pole attempt failed; the routed strategy contributed no solves (5 wins failed distillation and correctly reported as misses, 1 empty-vertex short-circuit); 9 winners were rejected by the archive policy (solved but not shelved); and zero decompositions fired, because the lower ladder never stalls long enough at depth 0 to justify recursion, so the decompose mechanism is exercised today only by its solvability-gate probes and unit tests.

**The fine-grained snapshot.** A second view comes from an interrupted sibling run at the same configuration on rungs 1 to 5 (32 tasks, 1,087 generations, 2,025 s; archived with its library under `ai/archive/20260705_g2/`), which started from a single-entry library holding the probe run's two-spirals stepping stone (Section 7.3). Its per-task rows show the cost collapse directly: double-pole solved once by evolution (55 generations, 172 s), then hit the library five consecutive times, twice at effectively zero cost (1 to 5 ms, a quick-evaluation short-circuit) and otherwise at 5.5 to 10.4 s dominated by optional refinement; across the run, a solved task cost 0.001 to 10 s against 2 to 715 s for evolution. Four xor-lineage entries were successively tombstoned by refinement dominance, leaving one survivor at metric 1.0. Its 14-entry library held a three-deep macro reuse chain (a level-3 parity module referencing a level-2 module referencing a level-1 module). The composition stage's cost on two-spirals grew from 72 to 671 s across attempts as stone lineages compounded, the deliberate opposite of restarting from scratch; that run's best two-spirals attempt reached 0.911, the highest single-attempt metric recorded on the family (stone-seeded, against the cold headline run's 0.901).

Two archived runs complete the picture. A 92-task run from an earlier system generation recorded `routed_solved = 30` with 16 zero-shot clears on a warmer library, so the routed mechanism can carry load even though its contribution under the current configuration is honestly zero; its evaluation on a mature library is future work. And a fully-warm 112-task run shows the memory steady state directly: 76 hits, 7 refinements, 4 evolutions, and 25 failures (all on the unsolved family), with rungs 1, 4, and 5 costing approximately nothing, and refinement at 15 attempts, 7 improvements, 68 decayed skips.
...
| Default config, cold (completed run, 67) | 0.901 | chance (67 of 67 rows) |
...
**The upper ladder is untested.** Production runs cover rungs 1 to 6, and rung 6 is unsolved (best 0.900 over 66 attempts). Rungs 7 to 10 load and run but have no campaign results; rungs 11 to 14 are blocked outright by the minimal initializer's dense input-output bipartite connect (10.8M genes at rung 11; gigabytes per genome at rungs 12 to 14), an algorithm-level wall requiring output-side factorization, a decompose-first policy, or the generative initializer's sparse seeding, whose payoff at exactly this scale is designed but unmeasured. Rungs 17 and 18 load and schedule but no results are claimed. ARC-AGI-3 remains a portability constraint (descriptor-only signatures, grid-to-grid decomposition, recurrence-ready substrate) with no interactive harness built.
...
**Scale unknowns.** The library index rewrites O(entries) per structural change and caches without bound; both carry explicit revisit watchpoints at thousands of entries. Router evaluation on large vertex sets, archive niche saturation dynamics, and motif-census value at scale are all unmeasured. The decompose-and-recurse mechanism, though tested, has never fired in a production run; the lower ladder does not need it, which means its real evaluation waits on harder rungs.
...
| evolve strategy | EVOLVE_STRATEGY | **routed**, **direct**, **composition** (ladder, in order) |
| init | INIT | **minimal**, cppn (generative) |
| selection | SELECTION | **tournament**, truncation, nsga2 |
| crossover | CROSSOVER | none, **neat** |
| mutation | MUTATION | 17 operators, 13 selected (Section 5.2) |
| train | TRAIN | none, **gradient** (composition/module trainer), **gradient_refine** (direct path), gradient_scheduled |
| population train | TRAIN_POPULATION | gradient_batched, gradient_refine (registered, disabled on M4 Max) |
| evaluate | EVALUATE | standard, weight_samples, **hybrid** |
| fitness | FITNESS | 14 components, weighted blend + optional Pareto objective vector |
...
| Group | Setting |
|---|---|
| Acceptance | query accuracy >= 0.95 (support fallback on query-less tasks) |
