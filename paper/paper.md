...
*Working draft, revised 2026-07-06. Prepared for submission; `paper_todo.md` and `ai/instructions_for_publish.md` enumerate the remaining experimental program.*
...
We are explicit about status: rungs 1 to 6 are exercised end to end today, with rungs 3 and 6 still unsolved under the orchestrated regime; rungs 12 to 14 are blocked by a known initialization wall, with rungs 11, 15, 17, and 18 marginal under the same arithmetic (Section 10); and ARC-AGI remains a portability constraint rather than a reported result. The companion document `paper_todo.md` enumerates the ablation and scaling program required before external claims.
...
| 14 | `psicov` | 58 | CONTINUOUS 271k to 5.1M -> CONTINUOUS 4.8k to 89k | NAS-Bench-360; widths vary per task |
...
Task counts and I/O widths were enumerated live from the published dataset on 2026-07-06 (the probe transcript is archived with the frozen artifacts). Widths are the flattened Level0 view that library signatures key on; temporal rungs additionally unroll the TIME axis under the stepped substrate. Rung difficulty is intrinsic to the data, not to any code path: higher rungs mostly mean wider flattened I/O and harder generalization, not new mechanisms. Live probes measure rung 11 (`spherical`) at 10,800 inputs by 100 outputs, rung 12 (`cosmic`) at 65,536 by 65,536, rung 13 (`darcy_flow`) at 177,241 by 177,241, and rung 14 (`psicov`) as per-task variable, spanning 271,377 to 5,095,857 inputs and 4,761 to 89,401 outputs across probed tasks. These rungs currently exceed the minimal-initialization regime (Section 10).
...
*Figure 1: The per-task solve ladder (upper tier) and the persistent memory beneath it (lower tier). A task escalates through library lookup, budgeted refinement on a hit, the three-strategy evolve ladder on a miss, and solvability-gated decomposition with recursion on a stall; every winner passes one admission gate. The library feeds every stage in return: lookup candidates, refinement seeds, frozen experts for routing, macros and modules for structural growth, and stepping-stone warm starts after failure.* Counters register only for mechanisms that are switched on, so a live-mode run's summary is byte-identical to one produced before the mechanism existed. This ledger is not bookkeeping polish; Section 8 recounts two multi-hour production stalls that were diagnosable only because stage timing and size metrics appear in every row.
...
**The upper ladder is untested.** Production runs cover rungs 1 to 6, and rung 6 is unsolved (best 0.900 over 66 attempts). Rungs 7 to 10 load and run but have no campaign results. Rungs 12 to 14 are blocked outright by the minimal initializer's dense input-output bipartite connect: rung 12's 65,536-by-65,536 contract alone implies over four billion connection genes per genome, and rung 14's widest task implies hundreds of billions. Rungs 11, 15, 17, and 18 sit in a marginal band of roughly 1.1M to 8.1M genes at initialization under the same arithmetic. This is an algorithm-level wall. Three remedies are implemented and registered (a rank-factored initialization that seeds an identity-bottleneck instead of the dense bipartite connect, a decompose-first policy that splits oversize tasks before the evolve ladder runs, and a flat-substrate output-width guard that routes wide-output tasks to the composition strategy's rank-factored glue), and a probe campaign over the wall rungs is configured; their payoff at exactly this scale is unmeasured as of this draft. Rungs 17 and 18 load and schedule but no results are claimed. ARC-AGI-3 remains a portability constraint (descriptor-only signatures, grid-to-grid decomposition, recurrence-ready substrate) with no interactive harness built.
...
| mutation | MUTATION | 23 operators, 13 selected (Section 5.2) |
...
Every number in Sections 7 and 8 traces to an immutable archived artifact under `ai/archive/<tag>/` beside the working tree (excluded from version control today; publishing these archives as a citable data deposit accompanies the code release): the completed headline run, its final library, the regenerated motif census, and the live rung-probe transcript under `20260706_flagship/` (with SHA256 sums); the gate arms under `20260705_g0/` and `20260705_g1/` (the latter holds both the cold run and the warm continuation); and the probe plus the stone-seeded snapshot under `20260705_g2/`. Each archive contains the full `run_summary.json` (one row per task, with outcome, metric, generations, wall seconds, per-stage seconds, champion size metrics, and the structure-versus-weights sample statistics) and the library state the run ended with.
