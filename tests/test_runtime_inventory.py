...
def test_profiles_have_the_declared_scale_and_hardware() -> None:
    ...
    assert canary["orchestrator"]["evolve"] == ["routed", "grammar", "spatial", "direct", "composition"]
    ...
    assert smoke["orchestrator"]["spatial"]["pop_size"] == 16
    assert smoke["orchestrator"]["spatial"]["train"]["steps"] == 5
    assert smoke["orchestrator"]["spatial"]["chunk_size"] == 32768
    ...
    assert full["orchestrator"]["spatial"]["pop_size"] == 128
    assert full["orchestrator"]["spatial"]["max_expanded_edges"] == 1000000
    assert full["orchestrator"]["spatial"]["chunk_size"] == 32768
