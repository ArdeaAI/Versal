...
from versal.tools.run_matrix import SOLVED_OUTCOMES, flatten_rows, load_summary
...
def build_command(config_path: Path, *, resume_dir: Path | None = None) -> list[str]:
    command = [sys.executable, "-m", "versal.main", "--config", str(config_path)]
...
def render_slurm_script(
    ...
    *,
    ...
    return "\n".join(
        (
            "#!/bin/bash",
            f"#SBATCH --job-name={slurm.get('job_name', 'versal-ablate')}",
