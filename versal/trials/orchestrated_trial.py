...
from versal import checkpoint, rendering, results
...
from versal.evolution.loop import HierarchicalLoop, HierarchicalState, state_from_dict, state_to_dict
from versal.evolution.multitask import PoolReport, TaskEntry, build_pool_report
from versal.evolution.registry import build_loop
from versal.evolution.schedule import build_schedule
from versal.external_archive import ArchiveManager, ExperimentLock
from versal.library import COMPOSITION, MODULE, ModuleLibrary, macro_resolver
...
class OrchestratedTrial(Proctor):
    ...
    def __init__(self, config: dict[str, Any], task: Any = None) -> None:
        ...
        from versal.substrate import set_macro_resolver
    ...
    def _release_evolution_task_adapters(self, orchestrator: Orchestrator) -> None:
        ...
        from versal.evolution.evolver import release_shared_task_adapters
    ...
    def _solve_isolated(
        ...
        if self.frozen_library_dir is None:
            return None
        with tempfile.TemporaryDirectory(prefix="versal_task_") as tmp:
            ...
            from versal.evolution.evolver import get_shared_pool, set_shared_pool
            from versal.substrate import set_macro_resolver
    ...
    def _run_gc(self, state: HierarchicalState) -> None:
        ...
        from versal.library import payload_refs
        ...
        if self.gc_removed and (router_dir / "router_meta.json").exists():
            from versal.tools.library_gc import prune_router
