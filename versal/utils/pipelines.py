...
Versal trials load their own Icarus data, so `Pipeline` only owns the ClearML task,
...
class Pipeline:
    ...
    def __init__(self, config: dict[str, Any], load_data: bool = False) -> None:
        ...
        self.project_name = config.get("project_name", "versal")
        ...
        if load_data:
            logger.warning("Pipeline.load_data is a no-op in Versal; trials load their own data.")
    ...
    def _create_task(self) -> None:
        ...
        self.task = Task.init(
            ...
            # Router checkpoints/shards are Versal state, not user-selectable model inputs.
