...
def _cached_library(path: str) -> "object":
    ...
    # the index from disk each time. versal.library imports nothing from this module, so this is safe.
...
def add_library_module(genome: Genome, ctx: MutationContext, *, rng: random.Random, prob: float = 0.05, path: str = "library") -> Genome:
    ...
    if rng.random() >= prob:
        return genome
    from versal.library import MODULE as MODULE_ENTRY
    from versal.library import ModuleLibrary
    ...
    from versal.library import is_field_entry
