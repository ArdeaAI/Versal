...
from versal.evolution.evaluate import standard as standard_evaluate
from versal.evolution.fitness import FitnessAggregator, stamp_complexity_metrics
from versal.evolution.genome import Genome, InnovationTracker, make_acyclic
from versal.evolution.mutation import AdaptiveMutationPipeline, MutationContext, MutationPipeline
from versal.evolution.novelty import NoveltyConfig, archive_insert, compute_descriptor, novelty_scores, probe_tensor
...
class TaskAdapter:
    ...
    def evaluate(self, module: SubstrateModule) -> dict[str, float]:
        from versal.structured import StructuredGridEncoded, evaluate_structured_grid
...
class Evolver:
    ...
    def assess_many(self, genomes: list[Genome], adapter: Adapter, state: EvolverState) -> list[Assessed]:
        ...
        from versal.evolution import train as train_stage
...
def _init_worker(library_dir: str) -> None:
    ...
    from versal.library import ModuleLibrary, macro_resolver
