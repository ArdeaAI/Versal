...
def build_loop(config: dict[str, Any]) -> Any:
    ...
    from versal.evolution import loop
...
def build_evolver(config: dict[str, Any]) -> "Evolver":
    ...
    if population_supported:
        from versal.utils.device import POPULATION_MODES, SERIAL_MODE, population_mode, resolve_compute_device, resolve_execution_mode
    ...
    if population_supported and use_population:
        from versal.utils.device import POPULATION_MODES, device_for_population_mode, resolve_compute_device
    ...
    if novelty_cfg is not None and bool(novelty_cfg.get("enabled", True)):
        from versal.evolution.novelty import NoveltyConfig
    ...
    from versal.utils.device import resolve_worker_count
